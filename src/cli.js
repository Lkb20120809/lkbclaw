...
function toolLines(tool, w) {
  ...
  const r = tool.result || {};
  if (r.diff) {
    if (r.replaced != null) out.push("  │ " + C.dim + `改动 ${r.replaced} 处` + C.reset);
    const lines = r.diff.split("\n");
    const MAX = 200;
    lines.slice(0, MAX).forEach((l) => {
      const c = l[0] === "+" ? C.add : l[0] === "-" ? C.del : C.dim;
      out.push("  │ " + c + escapeBlessed(l) + C.reset);
    });
    if (lines.length > MAX) out.push("  │ " + C.dim + `… 省略 ${lines.length - MAX} 行` + C.reset);
    return out;
  }
  if (showToolDetails) {
...
function renderConv() {
  ...
  if (turns.length === 0) {
    ...
    const logoW = Math.max(...LOGO.map((l) => displayWidth(l)));
    const pad = Math.max(0, Math.floor((innerW - logoW) / 2));
    const centeredLogo = LOGO.map((l) => C.brand + " ".repeat(pad) + l + C.reset);
    convLines = [
      ...Array(off).fill(""),
      ...centeredLogo,
      "",
